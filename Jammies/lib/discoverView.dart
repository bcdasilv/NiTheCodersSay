...
class _DiscoverView extends State<discoverView> {
  ...
  void post() async {
    ...
    var uri = Uri.parse('https://jam.smpark.in/makePost');

    Map<String, String> header = {'email': email, 'password': password};



    final response = await http.post('https://jam.smpark.in/makePost',
        headers: header,
  ...
  Future<List<Post>> populatePosts() async{
    ...
    final response = await http.get('https://jam.smpark.in/getPost', headers: header);
      ...
      postList.add(
        Card(
          ...
          Container(
            ...
            width: 150,
            child: FadeInImage(
              image: NetworkImage('https://jam.smpark.in/static/images/' + currPost.authorid.toString()), placeholder: AssetImage("assets/icon/icon.png"),
